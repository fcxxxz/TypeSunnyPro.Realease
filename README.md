...
| `TypeSunnyPro-版本号-macos-arm64.dmg` | Mac 用户 | Mac 用户请下载这个。把 `晴跟打Pro.app` 拖到“应用程序”，完成首次打开安全确认，再按应用内提示授权。只有首次打不开时才运行 DMG 中的 `.command` 文件。 |
| `TypeSunnyPro-版本号-macos-arm64.zip` | 应用内更新 | 给 macOS 应用内更新使用，普通用户不要优先下载。 |
| `*-package.json` | 程序更新器 | 更新器读取的包清单，普通用户不用下载。 |

### Mac 安装与输入监控授权

1. 把“晴跟打Pro.app”拖到“应用程序”。
2. 从“应用程序”打开晴跟打Pro，完成 macOS 的首次打开安全确认。如果打不开，再运行 DMG 中的 `2-首次打不开时运行.command`；仍被拦截时，到“系统设置 > 隐私与安全性”点击“仍要打开”。
3. 程序打开后，点击“启用输入监控”。macOS 会打开“输入监控”设置：如果列表里已有“晴跟打Pro”，直接打开右侧开关；如果列表里没有，点击左下角“+”，在“应用程序”中选择“晴跟打Pro”。应用内的“在访达中定位晴跟打Pro”可帮你快速找到正确 App。此权限只用于准确计算击键和码长，程序不会保存或上传你的按键内容。返回程序后会自动重新检测并恢复打字。

首个标准主进程自签名版本需要手动安装，并需要把“晴跟打Pro”添加到输入监控一次。后续版本只要继续使用同一签名证书和 `com.typesunny.pro`，覆盖升级不应重复授权。自签名没有 Apple Team ID，不能让 macOS 自动把 App 加入输入监控列表，也不能替代 Apple 公证或首次打开安全确认。
